...
import { Bot, Loader2, Trash2, History, Plus, MessageSquare, Send, User as UserIcon, Bell } from 'lucide-react';
import { GoogleGenAI } from '@google/genai';
import { JOB_NOTIFICATIONS_INSTRUCTION, TRANSLATIONS } from '../constants';
...
const JobNotifications: React.FC<JobNotificationsProps> = ({ theme, user, language }) => {
  ...
  const fetchSessions = async () => {
    try {
      const response = await fetch('/api/chat/history?type=JOB_NOTIFICATIONS', {
  ...
  const saveMessage = async (role: string, text: string) => {
    ...
    try {
      await fetch('/api/chat/save', {
        ...
        body: JSON.stringify({ sessionId: currentSessionId || Date.now().toString(), role, text, type: 'JOB_NOTIFICATIONS' }),
      });
      fetchSessions();
    } catch (error) {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
        ...
        contents: [
          {
            role: 'user',
            parts: [
              { text: JOB_NOTIFICATIONS_INSTRUCTION },
      ...
      const aiResponse = response.text || "Sorry, I couldn't find any jobs right now.";
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: 'model',
        text: "An error occurred while fetching job notifications.",
        timestamp: Date.now()
      }]);
    } finally {
      setLoading(false);
    ...
    <div className={`flex flex-col h-[calc(100vh-80px)] ${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold tracking-tight">{t.JOB_NOTIFICATIONS}</h2>
        </div>
        <div className="flex items-center gap-2">
          {user.id !== -1 && (
            <button
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50">
                <Bell className="w-16 h-16 text-[#D4AF37]" />
                <div>
                  <h3 className="text-xl font-medium">{t.JOB_NOTIFICATIONS}</h3>
                  <p className="text-sm">Ask me for the latest jobs or internships!</p>
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t border-white/10">
            <form onSubmit={handleSend} className="max-w-4xl mx-auto flex gap-2">
              <input
                ...
                placeholder="E.g., Find software engineering internships in Hyderabad..."
